...
import { addComment, deleteComment, updateComment, getComments, uploadCommentImage, deleteCommentImage } from "../lib/supabase";
...
export default function Comments({ blogId, userId, comments, onCommentUpdate }: CommentsProps) {
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      ...
      if (editImage) {
        const uploadedUrl = await uploadCommentImage(editImage, userId!);
        if (uploadedUrl) {
          finalImageUrl = uploadedUrl;
        }
      }

      // If image was removed (currentEditImageUrl is null and no new image), set to null
      if (!editImage && currentEditImageUrl === null) {
        finalImageUrl = null;
      }

      // Delete old image if it was replaced or removed
      if (originalImageUrl && (editImage || finalImageUrl === null) && originalImageUrl !== finalImageUrl) {
        await deleteCommentImage(originalImageUrl);
      }

      await updateComment(id, editText.trim(), finalImageUrl);
      setEditingId(null);
      setEditText("");
  ...
  const startEditing = (comment: any) => {
    ...
    setOriginalImageUrl(comment.image_url || null);
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  ...
                  >
                    ...
                    {currentEditImageUrl || editImagePreview ? "Change Image" : "Add Image"}
                  </button>

                  {editImagePreview && (
                    <div className="relative group">
                      <img src={editImagePreview} alt="Edit preview" className="w-12 h-12 object-cover rounded-lg border border-zinc-200 dark:border-zinc-700" />
                      <button
                        onClick={() => {
                          setEditImage(null);
                          setEditImagePreview(null);
                  ...
                  {currentEditImageUrl && !editImagePreview && (
                    <div className="relative group">
                      <img src={currentEditImageUrl} alt="Current image" className="w-12 h-12 object-cover rounded-lg border border-zinc-200 dark:border-zinc-700" />
                      <button
                        onClick={() => setCurrentEditImageUrl(null)}
                        className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[8px] shadow-lg"
                      >
                        ×
                      </button>
